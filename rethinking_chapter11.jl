...
# ╔═╡ 14e1cfbc-80b4-11f1-83bd-6d0587864a5d
using Turing, Random, Distributions, StatsPlots, StatsBase, DataFrames, CSV,LinearAlgebra, CategoricalArrays, MCMCChains, StatsFuns

# ╔═╡ a235ac22-0f02-4cdc-a123-d163d02cb1ce
## Binomial Regression
y0 = rand(Binomial(1,0.1))

# ╔═╡ b983506d-7b42-4af9-a079-fbbf8d3bad70
# load the chimpanzees data
begin
	chimps = "https://raw.githubusercontent.com/rmcelreath/rethinking/refs/heads/master/data/chimpanzees.csv"
	df = CSV.read(download(chimps), DataFrame)
	first(df, 3)
end

# ╔═╡ e0574d0e-ad59-4102-9cae-36e6016f4593
# Code 11_2
# Index variable treatment with prosoc_left and condition
df.treatment = 1 .+ df.prosoc_left .+ 2 .* df.condition 

# ╔═╡ f4bf825b-7e70-44c1-bbf8-935abb211f79
# Code 11_4
...
# ╔═╡ 0fbe4e88-aeb6-48ae-a03e-5a3e6e1193dd
missing_data = fill(missing, nrow(df))

# ╔═╡ cb2e7768-61ca-4951-b1dd-21fcaf529817
# sample from prior
prior_chain = sample(m11_1(missing_data), Prior(), 10_000)

# ╔═╡ fc7a508b-e149-4184-9fe8-38d3771c11f3
prior_chaindf = DataFrame(prior_chain)

# ╔═╡ 3055c8d7-f0d2-404b-8413-4fa997c8c966
# code 11_6
p0 = logistic.(prior_chaindf.a)

# ╔═╡ 96591ada-aee7-42c2-82bc-5580ac2d55e9
# density plot
density(p0, legend=false, title="Density Plot", bandwidth=0.01)

# ╔═╡ ccedcc42-6172-4655-9bdb-08eeabbbb75e
# code 11_7
@model function m11_2(pulled_left, treatment, num_treatments)
	a ~ Normal(0,1.5)
	b ~ filldist(Normal(0, 0.5), num_treatments)
	p = logistic.(a .+ b[treatment])
	for i in eachindex(pulled_left)
		pulled_left[i] ~ Binomial(1,p[i])
	end
	
end

# ╔═╡ 3d26230e-c21f-48ed-98a1-3c676d6ce320
begin
	num_treatments = 4
	missing_data2 = fill(missing, length(df.treatment))
	prior_chain2 = sample(m11_2(missing_data2, df.treatment, num_treatments),
					 Prior(),
					 10_000)
	prior_chain2df = DataFrame(prior_chain2)
	f = i -> @. logistic(prior_chain2df.a + prior_chain2df[!,"b[$i]"])
    p11_7 = map(f, 1:4);
end

# ╔═╡ fa61bc55-d8bf-4b88-a0d2-37045e462538
# code 11_8
density(abs.(p11_7[1] .- p11_7[2]), bandwidth=0.01,
	   legend=false)

# ╔═╡ 64e505d5-d92e-4f70-8665-1714b6b441bc
mean(abs.(p11_7[1] .- p11_7[2])) # about 10%

# ╔═╡ 4ac8b8ec-24b3-4e9d-af14-95596197c37e
df

# ╔═╡ 812a050a-9bdb-459a-8c13-d01371795490
df1 = select(df, [:pulled_left, :actor, :treatment])

# ╔═╡ ed1c44af-2521-409a-bc90-4166fd4c0a58
# code 11_9
@model function m11_3(pulled_left, actor, treatment, num_actors, num_treatments)
	a ~ filldist(Normal(0,1.5), num_actors)
	b ~ filldist(Normal(0, 0.5), num_treatments)
	p = logistic.(a[actor] .+ b[treatment])
	for i in 1:length(df1.pulled_left)
		pulled_left[i] ~ Binomial(1,p[i])
	end
end

# ╔═╡ 559eb2d3-0e2e-41d9-8320-721362cb82db
chain2 = sample(m11_3(df1.pulled_left, df1.actor, df1.treatment, 7, 4), NUTS(), 1_000)

# ╔═╡ 0ba8faef-698b-436d-a830-25be5ddca98e
chains2df = DataFrame(chain2)

# ╔═╡ 0205ea81-e30d-4769-bb11-34097d3e41d8
summarystats(chain2)

# ╔═╡ 75e861e7-ac5d-47b9-a625-86b667128444
p_left = DataFrame(map(i -> "V$i" => logistic.(chains2df[:,"a[$i]"]), 1:7)...)

# ╔═╡ aeb4b1d9-d023-44a7-9b85-8a149243ecb0
# UCB Admission data
begin
	ucbadmit = "https://raw.githubusercontent.com/rmcelreath/rethinking/refs/heads/master/data/UCBadmit.csv"
	# read the data
	dfucb = DataFrame(CSV.File(download(ucbadmit), skipto=2, 
        header=[:id, :dept, :gender, :admit, :reject, :applications]))
	first(dfucb, 3)
end

# ╔═╡ 4b8915b9-71a4-4598-9dd6-82341699b417
dat = select(dfucb, [:admit, :applications])

# ╔═╡ 85cd18aa-a188-42a6-b4d9-73f92f92c683
dat.gid = ifelse.(dfucb.gender .== "male", 1, 2)

# ╔═╡ ad5debbc-1c00-4922-8021-a5c6990cec69
dat

# ╔═╡ 59ff6a79-fa33-4735-8bb5-cb14bac250bb
# define the model
@model function m11_7(admit, applications, gid, num_gid)
	a ~ filldist(Normal(0, 1.5 ), num_gid)
	p = logistic.(a[gid])
...
# ╔═╡ a74690c9-ce08-4837-b828-ec37c6e81764
# run the sampler and chain
begin
	num_gid = 2
	chain3 = sample(m11_7(dat.admit, dat.applications, dat.gid, num_gid),
				   NUTS(),
				   1_000)
end


# ╔═╡ 9b0f01c1-75a1-40ee-b279-6b12ec9b93ec
summarystats(chain3)

# ╔═╡ 2efcfc6c-b750-46ee-9bad-18ed62b4d788
chain3df = DataFrame(chain3)

# ╔═╡ a0973c43-ca22-4a28-a86c-e276d4ffc97b
diff_a = chain3df."a[1]" .- chain3df."a[2]"

# ╔═╡ e5a5984e-09f8-45d1-a05c-04743367e157
diff_p = logistic.(chain3df."a[1]") .- logistic.(chain3df."a[2]")

# ╔═╡ 0ee97a47-7456-4f07-945c-bbc7f7708871
DataFrame(:diff_a => diff_a, :diff_p => diff_p)

# ╔═╡ dbd91e9b-d419-4699-ad3e-d512692874a2
md"""
...
# ╔═╡ 9e99ed46-1929-40a7-aeff-1bc0d0b9d26b
# code 11_35
y35 = rand(Binomial(1000, 0.001), 100_000)

# ╔═╡ 9689c1de-65ea-4dfb-88c1-b032e41f9d15
[mean(y35), var(y35)] # with Binomial, the mean and variance are identical

# ╔═╡ bdf13bb5-26d0-4532-8444-427ab843188a
# code 11_36
# read kline data
begin
	kline = "https://raw.githubusercontent.com/rmcelreath/rethinking/refs/heads/master/data/Kline.csv"
	dfk = CSV.read(download(kline), DataFrame)
	first(dfk, 3)
	rand(Poisson())
end

# ╔═╡ e6f3e69e-5d9d-4c16-b4ed-62e3c1caf601
first(dfk, 5)

# ╔═╡ 30ae2652-4179-4501-938e-8d4b6ecc5721
# code 11_37
begin
	dfk.pop = zscore(log.(dfk.population)) # zscore is the equivalent of scale
	dfk.contact_id = ifelse.(dfk.contact.=="high", 2,1)
end

# ╔═╡ 91f8c241-e31c-4d05-ad5c-c5d19f39cb08
begin
	plot(x -> pdf(LogNormal(3, 0.5), x), 0, 100, label="Theoretical PDF")
end

# ╔═╡ 697c0e54-8d37-4f31-963c-6c398a85c14b
# code 11_39
begin
	sim_a = rand(Normal(0,10), 10_000)
	lambda = exp.(sim_a) # always remember to add the dot for vectorised
	mean(lambda)
end

# ╔═╡ a8fe2524-78d6-4dd2-8041-5d9d58bbee63
# Code 11_41
begin
	n_41 = 100
	a_41 = rand(Normal(3,0.5), n_41)
	b_41 = rand(Normal(0,0.2), n_41)
	
end


# ╔═╡ 5e961db3-e744-4539-80bf-1ca4854c238c
begin
p_41 = plot(xlims=(0, 10), legend=false)
for i in 1:n_41
    # Pass an anonymous function to plot!
    plot!(p_41, x -> exp(a_41[i] + b_41[i] * x), color=:gray, alpha=0.3)
end
    p_41
end

# ╔═╡ 687623e7-1cfc-489a-a9be-4bf58bfed333
# code 11_45

# ╔═╡ fa4e74f9-c0d4-43c3-b922-4d1ef471d64a
begin
	# get the data sorted first
	dat45 = select(dfk, [:total_tools, :pop, :contact_id])
	
	first(dat45, 3)
end

# ╔═╡ 87c73fdb-18e9-434f-904a-714cfffca702
dat45

# ╔═╡ 7d8c9312-e813-4356-8853-d9f3de7f905e
# do the model
@model function m11_9(tools)
	a ~ Normal(3,0.5)
	lambda = exp.(a)
...
# ╔═╡ cb7b397c-2a2e-4c2e-8ecf-a9ad957b4c90
chain45 = sample(m11_9(dat45.total_tools), NUTS(), 1_000)

# ╔═╡ 9884c2b3-8dbc-4530-ad72-3ffb8520ac08
summarystats(chain45)

# ╔═╡ af2b682c-4c87-46d4-9c0b-06c3ef5e8d63
# interaction model

@model function m11_10(tools, cid, num_cid,pop)
	a ~ filldist(Normal(3,0.5), num_cid)
	b ~ filldist(Normal(0, 0.2), num_cid)
	
	lambda = exp.(a[cid] .+ b[cid] .* pop)
	for i in 1:length(tools) 
...
# ╔═╡ 4cc1892c-13f4-4c5c-a618-7926b03e8952
ncid = length(levels(dat45.contact_id)) # how many levels

# ╔═╡ cebc2bfc-9986-49b7-8db1-aba906e06e34
begin
	num_cid = 2
	chain10 = sample(m11_10(dat45.total_tools, dat45.contact_id, num_cid, dat45.pop), NUTS(), 1_000)
	summarystats(chain10)
end

# ╔═╡ 139ffaac-1ad2-4306-adcf-56887671548e
predict10 = predict(m11_10(dat45.total_tools, dat45.contact_id, num_cid, dat45.pop), chain10)

# ╔═╡ 1f0c31b5-289f-476c-8cd2-ddb1872dabdd
summarystats(predict10)
...
manifest_format = "2.0"
project_hash = "f70348a5cd920a033baf880951b98994d3138d41"
...
git-tree-sha1 = "86f74da0e378e681d7352968276dff49060458f1"
...
git-tree-sha1 = "d50dcb1b13d446b2738ca5e54a8327963c91b1b4"
...
git-tree-sha1 = "46ef4d02442bc5efd07f5edc374f3854b7781165"
