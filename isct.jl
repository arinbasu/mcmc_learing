...
# ╔═╡ 7104d32f-5e1b-4176-bd5e-858e0c2da2e9
# let's load the packages first
using DifferentialEquations, Random, Distributions, DataFrames, Plots, Statistics

# ╔═╡ ba9f5e2c-6c8e-11f1-a3ca-47d241d10ecb
md"""
## How to Conduct In silico clinical trials using Differential Equations in Julia

We are going to conduct a simple controlled clinical trial using Julia. We will test a drug X and a placebo pill Y for control of blood sugar among diabetics. We will conduct the trial following six steps as follows:

1. Set up a pharmacometric model first
2. Identify the parameters for the virtual population or in silico population
3. Set up the virtual population
4. Identify the parameters of the experiment or the trial (the "protocol")
5. Run the simulations
6. Analyse the data

In the scenario, we will test the efficacy of the drug X compared with placebo or control drug Y for reduction of blood sugar level among Type II diabetics. We will make the following assumptions:

Both drugs X and Y follow second order kinetics, so follow exponential decay, but at different rates. For X, k_x = 0.12 (stays in the body for a long time), and for Y k_y = 2.20 (very high and rapidly removed from the body), emax for X emax_x is about 2 mmol/l, but it can drop blood sugar level by 4 mmmol/l at its highest; while for Y emax_y will be about 0.5 - 1 mmol/l or it is not known to be effective at all. Correspondingly, we will assume the ec50 for X and Y, ec50_x will be about 1-2 mmol/l (that is at 50% of full dose it will still have 1-2 mmol/l effect), whereas for Y we will assume ec50_y will be about 0.10-0.15 mmol/l. These are somewhat arbitrary measurements, but this is a toy example to try out.

We will also assume that our patients are diabetics, so their average blood sugar levels are about 11 mmol/l, normally distributed and with about 4 mmol/l standard deviation. We will study with 30 mg tablets of both X and Y

Let's begin

"""

# ╔═╡ 4ebbeb33-19a3-4b5b-9fef-0bb4f0dedc3c
# Step 1. We will do the pharmacometrics
# write a function for calculation of the concentrations
function calculate_conc(u,p,t)
	k,emax,ec50,baseline = p # set the parameters
	return -k * u # the exponential equation
end

# ╔═╡ 9e9d490a-1ec9-49af-91f2-16275338e97c
# Now write another function for the calculation of sugar
function calculate_sugar(c,p)
	k,emax, ec50,baseline = p
	return baseline - (emax * c / (ec50 + c)) # this is the Hill's equation
end

# ╔═╡ 1ec0acb7-b57d-4594-97b2-494452e8208e
# Step 2. Set the population parameters
begin
	k_drug = LogNormal(log(0.12), 0.07) # log normal distribution
	k_control = LogNormal(log(1.01), 0.60) # same logic as above
	emax_drug = Uniform(2,4)
	emax_ctrl = Uniform(0.5,1)
	ec50_drug = Uniform(1,2)
	ec50_ctrl = Uniform(0.10, 0.15)
	d_baseline = Normal(11,4) # we keep this same for both groups
	
end

# ╔═╡ 611d2055-7423-4bbb-b60f-8df2e0abf8c4
x = [rand(LogNormal(log(1.01), 0.60)) for i in 1:1000]

# ╔═╡ bb1d36a0-6efa-48b5-b9ee-520b0a315859
plot(x, seriestype=:histogram, legend=false)

# ╔═╡ 1af7b209-0ad3-4dd3-a22e-ea3bcd042871
y = [rand(Uniform(1,4)) for i in 1:1000]

# ╔═╡ 1d0d6c0b-617a-43dd-b9d8-0128c80e9be1
plot(y, seriestype=:histogram, legend=false)

# ╔═╡ d78638e2-5b49-422e-9bb7-be482ad4e752
# Step 3. Now set up the virtual populations
begin
	n_pts = 1000
	virtual_drug = []
	virtual_ctrl = [] 
	for i in 1:n_pts
	drug_params = (
		k_drugs = rand(LogNormal(log(0.12), 0.07)),
		emax_drugs = rand(Uniform(2,4)),
		ec50_drugs = rand(Uniform(1,2)),
		drug_baseline = rand(Normal(11,4))
	)
	push!(virtual_drug, drug_params)	
	end

	for i in 1:n_pts
	ctrl_params = (
		k_ctrls = rand(LogNormal(log(1.01), 0.60)),
		emax_ctrls = rand(Uniform(0.5,1)),
		ec50_ctrls = rand(Uniform(0.10,0.15)),
		drug_baseline = rand(Normal(11,4))
	)
	push!(virtual_ctrl, ctrl_params)	
	end
	
	
end

# ╔═╡ d14ddba3-c276-4ceb-a629-7487909d82ef
# test
virtual_ctrl[1]

# ╔═╡ 935dc576-c4e3-4d72-b274-9a836b67fff5
# Step 4. Set up the study parameters
begin
	u0=30 # same for both drugs and control
	tspan=(0.0, 24.0) # same for both drugs and control
end

# ╔═╡ a5dfb58d-5468-4c59-9c90-13f5ae18a868
# Step 5. Run Simulations
# First set up a results data frame
results = DataFrame(
	ptid = Int[],
	grp = String[],
	baseline = [],
...
# ╔═╡ 3688be12-1a78-4be7-bfda-6bee1b63cff6
# Next, run the simulations first for the drug
for i in 1:n_pts
	p_drug = virtual_drug[i]
	baseline_drug = p_drug[4]
	prob_drug = ODEProblem(calculate_conc, u0, tspan, p_drug)
	sol_drug = solve(prob_drug)
	c_drug = sol_drug[end][1]
	final_drug = calculate_sugar(c_drug, p_drug)
	es_drug = baseline_drug - final_drug
	push!(results, [i, "Drug", baseline_drug, final_drug, es_drug])
	
end

# ╔═╡ fd8d456e-4592-42e0-b9aa-fdcaa6049de3
results

# ╔═╡ 39c00f81-09a8-4d8a-b923-6fc194958c6e
# Finally, run the simulations for controls
for i in 1:n_pts
	p_ctrl = virtual_ctrl[i]
	baseline_ctrl = p_ctrl[4]
	prob_ctrl = ODEProblem(calculate_conc, u0, tspan, p_ctrl)
	sol_ctrl = solve(prob_ctrl)
	c_ctrl = sol_ctrl[end][1]
	final_ctrl = calculate_sugar(c_ctrl, p_ctrl)
	es_ctrl = baseline_ctrl - final_ctrl
	push!(results, [i, "Control", baseline_ctrl, final_ctrl, es_ctrl])
	
end


# ╔═╡ f853a0bf-a653-41fd-9a22-ccfbcad2e696
# Step 6. Analyse the data set for effect size
# First separate the two data sets
begin
drugset = subset(results, :grp => ByRow(x -> x == "Drug"))
ctrlset = subset(results, :grp => ByRow(x -> x == "Control"))
end

# ╔═╡ b00f9fd0-4cb5-4357-af98-1946dc83d682
# Next, calculate the mean effect and sd for effect_size for drugs
[ mean(drugset.effect_size), std(drugset.effect_size) ]

# ╔═╡ d1be6661-4482-40c4-95e6-780a52d58fbe
# mean effect and sd for effect_size for controls
[ mean(ctrlset.effect_size), std(ctrlset.effect_size) ]

# ╔═╡ 68233c91-434c-42d9-9b10-a218dd438f38
md"""
## Summary
So, the above set of codes show how to conduct a comparative trial based on emax, ec50, and k values obtained from various sources for pharmaceutical agents to study efficacy of these compounds for specific clinical outcomes. The process closely approximates randomised controlled trials, as you can flexibly vary the number of participants, and can vary the number of parameters. 
...
manifest_format = "2.0"
project_hash = "f2536a4b8fbff0dedb0c723a69245f0d5f120b04"
